...
# POS Multi-Shop
    multi_store = fields.Boolean(string='Multi Shop')
...
# POS Reorder end here
    ...
    store_id = fields.Many2many("pos.store",string="Shop")
...
#         product_records = self.env['product.product'].sudo().search_read(domain,product_fields)
...
#         return product_records
    ...
    def get_products_category_data(self,flag_config):
        ...
        if self.store_id and self.store_id.location_id.category_ids:
            for cat in self.store_id.location_id.category_ids:
                category_list.append(cat.id)
        dummy_products = self.env['product.product'].sudo().search([('is_dummy_product','=',True)]).ids
        if flag_config == False:
