...
class account_asset_disposal(models.Model):
    ...
    def create_disposal_invoice(self):
        ...
        # print "inv", inv
        # public=self.env.ref('')
        inv_id = self.env['account.invoice'].sudo().create(inv)
        p = False
...
#        'netbook_value': fields.function(_net_get, digits_compute=dp.get_precision('Account'), store= {'asset.disposal': (lambda self, cr, uid, ids, c={}: ids, [], 10)}, string='Net Book Value', type='Float'),
    disposal_value = fields.Float(string='Disposal Amount ', digits_compute=dp.get_precision('Account'), required=True, readonly=True, states={'draft':[('readonly', False)]})
...
#        'value_residual': fields.related('asset_id', 'value_residual', type='Float', string='Net Book Value', digits_compute=dp.get_precision('Account'), readonly=True, states={'draft':[('readonly',False)]}),
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True, readonly=True, states={'draft':[('readonly', False)]}, default=lambda self: self.env['res.company']._company_default_get('asset.disposal'))
    ...
    asset_id = fields.Many2one('account.asset.asset', string='Asset', required=True, readonly=True, domain=[('method_asset', '=', 'new'), ('state', '=', 'open')], states={'draft':[('readonly', False)]})
