...
class repair_program(models.Model):
    ...
    asset_id = fields.Many2one('account.asset.asset', string='Asset', domain=[('method_asset', '=', 'new')], required=True, readonly=False)
...
class account_asset(models.Model):
    ...
    def _get_current_depre(self):
        for record in self.depreciation_line_ids:
            ...
            data = record.depreciation_date.split("-")[0]
            # print data
...
#        'recompute_asset': fields.Boolean('Recompute', help='Tick if you want to update the gross value of asset and recompute the depreciation with new gross value.'),
    ...
    add_method_period = fields.Integer(string='Period Length', required=True, readonly=True, states={'draft':[('readonly', False)]}, help="State here the time during 2 depreciation, in months to depreciation", default=5)
...
class account_asset_capitalize(models.Model):
    ...
    method = fields.Selection(selection=[('repair', 'Repair to Existing Asset'), ('main', 'Maintenance of Asset')], string='Method', required=True, readonly=True, states={'draft':[('readonly', False)]}, default='repair')
    name = fields.Char(string='Name', required=True, readonly=True, states={'draft':[('readonly', False)]})
    user_id = fields.Many2one('res.users', string='Responsible User', required=True, readonly=True, states={'draft':[('readonly', False)]}, default=lambda self: self.env.user)
    company_id = fields.Many2one('res.company', string='Company', required=True, readonly=True, states={'draft':[('readonly', False)]}, default=lambda self:self.env['res.company']._company_default_get('asset.capitalize'))
    date = fields.Date(string='Date', required=True, default=time.strftime('%Y-%m-%d'), states={'approve':[('readonly', True)], 'reject':[('readonly', True)], 'cancel':[('readonly', True)]})
    ...
    asset_id = fields.Many2one('account.asset.asset', string='Asset', domain=[('method_asset', '=', 'new')], required=True, readonly=True, states={'draft':[('readonly', False)]})
    cost = fields.Float(string='Cost', digits_compute=dp.get_precision('Account'), help='Cost for Maintenance/Repairs based on method set.', required=True, readonly=True, states={'draft':[('readonly', False)]})
#        'repair_name = fields.char('Repair Name', size=64, readonly=True, states={'draft':[('readonly',False)]}),
    ...
    product_uom = fields.Many2one('product.uom', string='Duration UoM', help="UoM (Unit of Measure) is the unit of measurement for Duration", readonly=True, states={'draft':[('readonly', False)]})
    ...
    mproduct_uom = fields.Many2one('product.uom', string='Duration UoM', required=False, help="UoM (Unit of Measure) is the unit of measurement for Duration", readonly=True, states={'draft':[('readonly', False)]})
    ...
    _constraints = [
        (_check_threshold_capitalize, 'Can not create repair/maintenance as cost of repair is below than capitalization threshold configured on asset', ['cost']),
    ]
