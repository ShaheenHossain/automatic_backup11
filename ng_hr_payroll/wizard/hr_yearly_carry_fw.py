...
class yearly_carry_fw(models.Model):
    ...
    date_from = fields.Date(string='Start Date', required=True, help='Start date of last year', default=time.strftime('%Y-01-01'))
    date_to = fields.Date(string='End Date', required=True, help='End date of last year')
    type = fields.Selection(selection=[('all', 'All'), ('none', 'None'), ('few', 'Number Of Days')], string='Type', required=True, default='all')
    ...
    def carry_fw(self):  # already override to ng_hr_hgolidays module..
        ...
        allocation_ids = holiday_obj.search([('holiday_type', '=', 'employee'), ('state', 'in', ('validate', 'validate1')), ('type', '=', 'add'), ('holiday_status_id', 'in', type_ids.ids), ('carry_fw', '=', False)])
        type_ids = []
...
#                            leaves[e.id][t.holiday_status_id.id] += t.number_of_days_temp
        ...
        leave_ids = holiday_obj.search([('date_from', '>=', res['date_from']), ('date_to', '<=', res['date_to']), ('type', '=', 'remove'), ('holiday_status_id', 'in', type_ids), ('state', 'in', ('validate', 'validate1'))])
        leaves1 = {}
        hlist = []
        for l in leave_ids:
        ...
        for t, m in leaves.items():
            for k, s in m.items():
                if not t in leaves1:
                    name = 'Carry Forwarded ' + self.env['hr.holidays.status'].browse(k)[0].name
                    vals = {'employee_id':t, 'holiday_status_id': k, 'type':'add', 'name': name , 'carry_fw': True, 'number_of_days_temp': s, 'notes': 'Carry Forward leave' }
                    ...
                    if leaves1[t][k] < s:
                        ...
                        vals = {'state':'draft', 'employee_id':t, 'holiday_status_id': k, 'type':'add', 'name': name, 'carry_fw': True, 'number_of_days_temp': diff, 'notes': 'Carry Forward leave' }
