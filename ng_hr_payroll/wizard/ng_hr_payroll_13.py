...
class hr_payslip_bonus(models.Model):
    ...
    merge = fields.Boolean(string='Consolidate Salary', help='Tick if you do not want to create separate payslip line for 13th month salary')
    percent = fields.Float(string='Percentage', digits=dp.get_precision('Payroll'), help='% should be between 0-1 for eg 0.10 for 10%')
    employee_ids = fields.Many2many('hr.employee', 'hr_employee_bonus_rel', 'bonus_id', 'employee_id', string='Employees')
    base = fields.Selection(selection=[('BASIC', 'Based on Basic'), ('GROSS', 'Based on Gross'), ('NET', 'Based on Net'), ('gross_per', 'Based on % Gross'), ('net_per', 'Based on % Net')], string='Based On', required=True, default='BASIC')
    ...
    def compute_sheet(self):
        ...
        if not company_id:
            raise Warning( _('Your company has no policy to give 13th Month Salary, please check your company configuration under setting menu'))
        data = self.read()[0]
